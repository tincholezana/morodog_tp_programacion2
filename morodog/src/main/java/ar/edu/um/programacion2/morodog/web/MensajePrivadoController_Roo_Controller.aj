...
privileged aspect MensajePrivadoController_Roo_Controller {
    ...
    @Autowired
    UsuarioService MensajePrivadoController.usuarioService;
    ...
    public String MensajePrivadoController.createForm(Model uiModel) {
        ...
        if (usuarioService.countAllUsuarios() == 0) {
            dependencies.add(new String[] { "idUsuarioEmisor", "usuarios" });
        }
    ...
    public String MensajePrivadoController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("mensajeprivadoes", MensajePrivado.findMensajePrivadoEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) mensajePrivadoService.countAllMensajePrivadoes() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("mensajeprivadoes", MensajePrivado.findAllMensajePrivadoes(sortFieldName, sortOrder));
        }
        addDateTimeFormatPatterns(uiModel);
